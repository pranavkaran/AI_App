...
public class UnitStrategy {
	...
	public void Unit_Process(){
		setDatabase();
		System.out.println(database);
		startProcess();
		System.out.println("UnitStrategy");
		displayFinalDatabase();
	}
	
	public void startProcess() {
		int firstPointer = 0;//database.size() - 1;
		int secondPointer = 0;
		...
		while(endLoop(firstPointer,secondPointer, database.size())){
			...
			while(firstPointer == secondPointer ? false : true){
				...
				String strResolvent = objAI.Resolvent(database.get(secondPointer), database.get(firstPointer), "");
